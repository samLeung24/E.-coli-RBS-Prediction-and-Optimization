### À ma chérie? La lapine blanche

library(Biostrings)

# define the SD sequence
sd_template <- DNAString("AGGAGG")
...
for (i in 1:length(fasta_sequences)) {
  ...
  sd_matches <- matchPattern(sd_template, substring(seq, 1, 20), max.mismatch = 3, min.mismatch = 0)

  # if SD sequence is found, adjustable spacer threshold
  if (length(sd_matches) > 0 & (20 - end(sd_matches)[1] == 6 | 
                                20 - end(sd_matches)[1] == 7 |
                                20 - end(sd_matches)[1] == 8))  {
    # get start and end positions of SD sequence
    sd_start[i] <- start(sd_matches)[1]
    sd_end[i] <- end(sd_matches)[1]
    sd_seq[i] <- substring(seq,start(sd_matches)[1],end(sd_matches)[1])
...
# write output to csv
write.csv(output_df, file = "predictionBioString3.csv", row.names = FALSE)
