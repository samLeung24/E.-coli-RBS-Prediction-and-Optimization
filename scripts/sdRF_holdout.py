...
maxlen = df['sd_seq'].apply(len).max()
df['paddedSD'] = pad_nucleotide_sequences(df['sd_seq'], maxlen=maxlen)
...
# Evaluate the best training set
...
plt.text(0.95, 0.95, f"R={R_train:.3f}", ha='right', va='top', transform=plt.gca().transAxes, fontsize=12)
plt.show()
